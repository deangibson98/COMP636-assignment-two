...
@app.route("/listbooks") #booklikst.html
def listbooks():
...
@app.route("/loanbook") #addloan.html
def loanbook():
...
@app.route("/loan/add", methods=["POST"]) #addloan.html & currentloans.html
def addloan():
...
@app.route("/listborrowers") #borrowlist.html
def listborrowers():
    connection = getCursor()
    connection.execute("SELECT * FROM borrowers;")
    borrowerList = connection.fetchall()
...
def currentloans(): #currentloans.html
    connection = getCursor()
